  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              <span className="font-extrabold text-lg sm:text-xl tracking-tight text-slate-900 dark:text-white leading-tight">
                Smart Promo <span className="text-blue-600 dark:text-blue-400 font-semibold text-xs tracking-normal uppercase px-1.5 py-0.5 rounded bg-blue-50 dark:bg-blue-950/80 border border-blue-200 dark:border-blue-800/80 ml-1">LLC</span>
              </span>
                ...
                <Link
                  ...
                  className={`px-3.5 py-2 rounded-lg text-sm font-medium transition-colors ${
                    isActive
                      ...
                      : "text-slate-600 hover:text-blue-600 dark:text-slate-300 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800/50"
                  }`}
                >
                  {link.label}
                </Link>
          ...
          <div className="hidden lg:flex items-center space-x-3">
            <a
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden pt-4 pb-6 border-t border-slate-200 dark:border-slate-800 mt-3 space-y-2 animate-fadeIn">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-between px-3 py-2.5 rounded-lg text-base font-medium text-slate-800 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800"
              >
