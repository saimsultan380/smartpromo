...
export default function Pricing({ onSelectTier }: PricingProps) {
  ...
  return (
    <section id="pricing" className="py-20 lg:py-28 bg-slate-50 border-t border-slate-200/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            return (
              <div
                ...
                className={`relative flex flex-col justify-between rounded-3xl p-8 sm:p-10 transition-all duration-300 ${
                  tier.isPopular
                    ? "bg-white border-2 border-blue-600 shadow-2xl shadow-blue-500/10 scale-100 lg:-translate-y-2"
                    : "bg-white/80 border border-slate-200/90 shadow-sm hover:shadow-xl hover:border-slate-300"
                }`}
              >
                {/* Popular Pill Badge */}
                {tier.isPopular && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-gradient-to-r from-blue-700 via-blue-600 to-indigo-600 text-white text-xs font-bold uppercase tracking-wider shadow-md">
        ...
        <div className="mt-14 p-6 rounded-2xl bg-white border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-bold text-slate-900 text-sm">Need Multi-Entity or High-Volume Custom Architecture?</h4>
              <p className="text-xs text-slate-500">Customized scopes available for VC-backed startups, M&A prep, and conglomerates.</p>
            </div>
          </div>

          <button
            onClick={() => onSelectTier("custom-enterprise")}
            className="shrink-0 px-5 py-2.5 rounded-xl bg-slate-900 text-white font-semibold text-xs hover:bg-slate-800 transition-colors"
          >
